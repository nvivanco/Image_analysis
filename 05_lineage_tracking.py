...
def run_lineage_tracking(base_dir, model_path, strain_dict_json, prob_threshold, output_filename):
	...
	dataset_root = os.path.join(base_dir, 'gnn_dataset')
	# Use the current time stamp for a unique directory name to avoid conflicts
	# NOTE: You might want to add a unique folder name here for repeatability/stability
	dataset = bh_track.CellTrackingDataset(root=dataset_root,
										   df_cells=df_all,
...
def plot_kymographs(base_dir, df_for_kymograph_plot):
	...
	for index, cell_view in unique_views.iterrows():
		...
		# NOTE: Using the simple naming convention (e.g., '007_992.tif') used in your original Stage 4 logic
		path_to_phase_kymograph = os.path.join(kymo_base_path, f'{fov}_{trench}.tif')
		path_to_fluor_kymograph = os.path.join(kymo_base_path, 'fluor',
